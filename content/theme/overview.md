...
- **Vector embeddings**: Content is indexed with 1024-dimension embeddings
- **Repo default**: The bundled setup uses libsql-search configured as `provider: 'cloudflare'`, serving `@cf/baai/bge-m3` through Workers AI
...
```bash
# Cloudflare Workers AI - required for indexing and search
CLOUDFLARE_ACCOUNT_ID=your-account-id
CLOUDFLARE_API_TOKEN=your-api-token

# Remote libSQL / Turso - optional
TURSO_DB_URL=libsql://your-db.turso.io
TURSO_AUTH_TOKEN=your-token
```

When the Turso values are not set, the project falls back to `file:local.db`.
The Workers AI credentials have no fallback: embeddings are generated by an
external service, so indexing exits non-zero and search returns 500 without
them.
...
Indexing calls Workers AI, so set the credentials first:

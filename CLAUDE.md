...
- **scripts/index-content.ts**: Indexes markdown files into `articles_cf_bgem3_1024` using Cloudflare Workers AI embeddings (1024 dimensions)
- All content queries use functions from libsql-search: `getAllArticles()`, `getArticleBySlug()`, `getArticlesByFolder()`, `getFolders()`

### Environment Variables
Required in `.env` for indexing and search:
- `CLOUDFLARE_ACCOUNT_ID`: Cloudflare account ID for Workers AI
- `CLOUDFLARE_API_TOKEN`: API token with the Workers AI read permission
...
Embeddings are generated by Cloudflare Workers AI (`@cf/baai/bge-m3`). There is no
in-process embedding path: `pnpm index` exits non-zero without credentials, and the
search endpoint returns 500. Only `pnpm db:init` works without them. Indexed content
and every search query are sent to Cloudflare.
...
4. Maintain embedding dimension consistency (1024) across schema creation, indexing, and search

### Customizing Embeddings
`src/lib/searchConfig.ts` is the single source of truth for the table name, vector
width, and provider options; `scripts/init-db.ts`, `scripts/index-content.ts`, and the
search API all read from it.

The width is 1024 because `@cf/baai/bge-m3` is fixed at 1024 — it is not a free
parameter while the provider is `cloudflare`. Changing provider, model, or dimensions
requires a new table plus a full reindex; two embedding spaces must never share a
table. The current table is `articles_cf_bgem3_1024`.
...
- When upgrading from an older index (`articles` at 768, or `articles_local_384`), run `pnpm db:init` before `pnpm index` to create and populate `articles_cf_bgem3_1024`; those vectors are in a different embedding space and cannot be reused. Retire the old tables separately after validation
- Set environment variables (`CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_API_TOKEN`, `TURSO_DB_URL`, `TURSO_AUTH_TOKEN`) in your deployment platform's dashboard
